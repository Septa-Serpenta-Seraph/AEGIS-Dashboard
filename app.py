...
def list_free_models():
    ...
    return jsonify({'success': True, 'models': models, 'count': len(models), 'auto_model': 'openrouter/free'})

@app.route('/api/autonomy/supervisor-model', methods=['GET'])
def get_supervisor_model():
    return jsonify({'success': True, 'model': _supervisor_model})
...
def set_supervisor_model():
    ...
    return jsonify({'success': True, 'model': _supervisor_model, 'message': f'Supervisor model set to {model}'})
...
def supervisor_chat():
    ...
    try:
        if docker_client:
            containers = docker_client.containers.list()
            if containers:
                container_info = [f"- {c.name}: {c.status} ({c.image.tags[0] if c.image.tags else 'unknown'})" for c in containers]
                system_context += "\nActive containers:\n" + "\n".join(container_info)
    except Exception as e:
...
Be concise and technical. Use bullet points for lists. If asked about status, reference the data above."""
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'response': 'Request timed out. The model may be overloaded — try again or switch model.',
